...
export function colinkRequestFromPatient(patient: Patient): ColinkRequest {
  return {
    ...
    address: `${patient.address?.addressDetail} ${patient.address?.building} ${patient.address?.village} ${patient.address?.moo} ${patient.address?.soi} ${patient.address?.road}`,
    subdistrict_code: patient.address?.subDistrict,
    district_code: patient.address?.district,
    province_code: patient.address?.province,
    nationality: undefined,
    address_zone: patient.address?.bangkokZone,
    test_type: undefined,
